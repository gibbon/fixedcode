...
describe.skipIf(!canRun)('e2e AI sandwich: draft → generate → enrich', () => {
  ...
  const exampleSpec = readFileSync(
    join(SPRING_DOMAIN_BUNDLE, 'examples', 'order-domain.yaml'),
    'utf-8'
  );
  ...
  it('full pipeline: draft spec → generate code → enrich extension points', async () => {
    const specPath = join(tmpDir, 'order-domain.yaml');
    ...
    const yamlResult = await draft({
      kind: 'spring-domain',
      description: 'order service with orders and lineItems',
      output: specPath,
      configPath,
      retry: false,
    });
    ...
    expect(yamlResult).toContain('Order');
    ...
    const hasAggregate = allFiles.some(f => f.includes('Order.kt'));
    const hasCommands = allFiles.some(f => f.includes('CreateOrderCommand.kt'));
    const hasApi = allFiles.some(f => f.includes('OrderApiDelegateImpl.kt'));
    const hasTest = allFiles.some(f => f.includes('OrderTest.kt'));
    ...
    expect(extensionPoints.some(f => f.includes('DefaultOrderBusinessService'))).toBe(true);
    expect(extensionPoints.some(f => f.includes('DefaultOrderValidator'))).toBe(true);
