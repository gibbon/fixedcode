...
export async function publishPackage(options: PublishOptions): Promise<string> {
  ...
  if (!repo) {
    try {
      repo = execSync('git remote get-url origin', { cwd: options.packageDir, encoding: 'utf-8' }).trim();
  ...
  const author = pkgJson.author ?? (() => {
    try { return execSync('git config user.name', { encoding: 'utf-8' }).trim(); } catch { return 'unknown'; }
  })();
  ...
  try {
    ...
    const prUrl = execSync(
      `gh pr create --repo ${registryRepo} --head ${branchName} --title "${commitMsg}" --body "${prBody.replace(/"/g, '\\"')}"`,
