...
Example prompt: "I need a order management service with CRUD operations for orders and lineItems"
→ AI produces `order-domain.yaml` with aggregates, commands, queries, events.
...
2. These are the user-owned files (e.g. `DefaultOrderBusinessService.kt`, `DefaultOrderValidator.kt`)
...
fixedcode verify order-domain.yaml build  # 21 checks passed
