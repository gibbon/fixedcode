...
import {
  generateFiles as domainGenerateFiles,
  enrich as domainEnrich,
} from '../../bundles/spring-domain/src/index.js';
...
async function main() {
  ...
  const libCtx = libraryBundle.enrich(libSpec.spec, {
    name: libSpec.metadata.name,
    apiVersion: libSpec.apiVersion,
  });
  ...
  const domCtx = domainEnrich(domSpec.spec, {
    name: domSpec.metadata.name,
    apiVersion: domSpec.apiVersion,
  });
  ...
  console.log(
    `=== Total: ${libRendered.length + domEntries.length} files generated to ${outputDir} ===`,
  );
} // end main
